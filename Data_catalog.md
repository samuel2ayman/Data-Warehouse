...
| Gender | VARCHAR | NULLABLE | Customer's gender. *Example: `F`* |
| Marital_Status | VARCHAR | NULLABLE | Customer's marital status. *Example: `M` (Married) / `S` (Single)* |
...
| Marital_Status | VARCHAR | NULLABLE | Product lifecycle status (active / discontinued). *Example: `Active`* |
| Maintenance | VARCHAR | NULLABLE | Maintenance tier or service requirements. *Example: `Standard`* |
...
## Key Metrics Derivable from This Schema

| Metric | Formula |
|---|---|
| Total Revenue | `SUM(Fact_Sales.Sales_Amount)` |
| Total Units Sold | `SUM(Fact_Sales.Quantity)` |
| Average Order Value | `AVG(Fact_Sales.Sales_Amount)` |
| Gross Margin | `SUM(Sales_Amount) - SUM(Quantity × Cost)` |
| Revenue by Country | Join `Customer_Dim.Country` |
| Revenue by Product Line | Join `Product_Dim.Product_Line` |
| Sales by Category | Join `Product_Dim.Category_Id / Sub_Category` |
| Order Fulfillment Time | `DATEDIFF(Ship_Date, Order_Date)` |

---

## Notes & Data Quality Considerations

- `Order_Number` in `Fact_Sales` is the **surrogate key** — a system-generated identifier that uniquely identifies each sales transaction in the warehouse layer.
- `Marital_Status` appears in **both** `Customer_Dim` and `Product_Dim`. In `Product_Dim`, this column refers to product lifecycle status (e.g., Active / Discontinued) and should be renamed (e.g., `Product_Status`) to avoid confusion.
- Date columns (`Ship_Date`, `Due_Date`) in `Fact_Sales` may be nullable to handle orders not yet shipped or fulfilled.
- Surrogate keys (`Customer_Key`, `Product_Key`) should always be used for joins — never join on natural keys (`Customer_Id`, `Product_Id`) in the Gold layer.
- This schema does not currently include a **Date Dimension** (`Date_Dim`). Adding one linked to `Order_Date`, `Ship_Date`, and `Due_Date` is strongly recommended for robust time-series analysis.

---

*Last Updated: April 2026 | Owner: Data Engineering Team*
